...
local function basedir(dir)for f in LuaMan:GetFileList(dir)do if string.find(f,".lua")then basefile(dir..f,function(dir)if dir then dir = dir:gsub("/", ".");require(dir:gsub("Mods/Extensions/",""):gsub("%.lua$",""))end end)end end;for d in LuaMan:GetDirectoryList(dir)do if d~=""then basedir(dir..d.."/")end end end
