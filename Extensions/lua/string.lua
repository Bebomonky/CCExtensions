...
--[[---------------------------------------------------------
	Name: UnFormattedTime( Time )
	Desc: returns unformatted time into seconds.
		refer to string.FormattedTime() for more information
------------------------------------------------------------]]
function string.UnFormattedTime( time )
	if ( not time ) then return debug.traceback(tostring(time)) end
	local seconds = time.s
	local hours = time.h * 3600
	local minutes = time.m * 60
	local millisecs = time.ms / 100
...
function string.Comma( number, str )
	...
	if ( str ~= nil and not type( str ) == "string" ) then
