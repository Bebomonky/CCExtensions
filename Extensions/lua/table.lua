...
function table.Merge( dest, source, forceOverride )
	...
	for k, v in pairs( source ) do
		if ( not forceOverride and type( v ) == "table" and type( dest[ k ] ) == "table" ) then
	...
	for k, v in pairs ( t ) do
		...
		if ( type( v ) == "table" and not done[ v ] ) then
			...
			if ( type( v ) == "boolean" ) then
				...
				tbl[ k ] = { __type = "Boolean", tostring( v ) }
...
if ( not table.move ) then
	function table.move( sourceTbl, from, to, dest, destTbl )
		...
		if ( not type( sourceTbl ) == "table" ) then error( "bad argument #1 to 'move' (table expected, got " .. type( sourceTbl ) .. ")" ) end
		if ( not type( from ) == "number" ) then error( "bad argument #2 to 'move' (number expected, got " .. type( from ) .. ")" ) end
		if ( not type( to ) == "number" ) then error( "bad argument #3 to 'move' (number expected, got " .. type( to ) .. ")" ) end
		if ( not type( dest ) == "number" ) then error( "bad argument #4 to 'move' (number expected, got " .. type( dest ) .. ")" ) end
		if ( destTbl ~= nil ) then
			if ( not type( destTbl ) == "table" ) then error( "bad argument #5 to 'move' (table expected, got " .. type( destTbl ) .. ")" ) end
		else
			destTbl = sourceTbl
		end
